...
async function runPostStageActions(client: any, taskId: string, to: Stage): Promise<SideEffect[]> {
  ...
  if (to === 'analysis_done') {
    ...
    const autoTitle = `Test Plan Draft - ${row.title}`;
    ...
    if ((exists.rowCount || 0) === 0) {
      ...
      await client.query(
        ...
        [
          ...
          nextLevel,
          autoTitle,
          'Auto-generated by workflow trigger. Please add test scope, test cases, and acceptance criteria.',
          'pending',
          0,
          'PHP',
          'auto-trigger',
...
export async function moveStage(taskId: string, to: Stage, eventType: string, actor?: string, payload?: Record<string, unknown>) {
  ...
  try {
    ...
    if (!rs.rows[0]) throw new Error('Task not found');
    const from = rs.rows[0].workflow_stage as Stage;
    if (!canMove(from, to)) throw new Error(`Invalid state transition: ${from} -> ${to}`);
    ...
    for (const effect of committedSideEffects) {
      try {
        ...
        logger.error(`[workflow-stage] Compensation failed: ${effect.description}`, compErr, { taskId, to });
      }
    }
