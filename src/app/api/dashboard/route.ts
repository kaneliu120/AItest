...
export async function GET(request: Request) {
  ...
  if (fd?.monthlyTrend?.length) {
    ...
    totalExpense = sorted.reduce((sum: number, item) => sum + (item.expenses || 0), 0);
  ...
  // ── Freelance ─────────────────────────────────────────────────────────────
  const projects = ((freelanceData as any)?.projects ?? (freelanceData as any)?.data?.projects ?? []) as Array<Record<string, any>>;
  const activeProjects = projects.filter((p) => p.status === "active").length;
  const pendingProposals = projects.filter(
    (p) => p.status === "pending" || p.status === "proposal"
  ).length;
  const avgProgress = projects.length > 0 
    ? Math.round(projects.reduce((sum: number, p) => sum + (p.progress || 0), 0) / projects.length)
    : 0;
