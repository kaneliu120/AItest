...
export async function GET(request: Request) {
  ...
  const usageTrend = Array.from({ length: 7 }).map((_, i) => {
    ...
    return { day: key, usageCount: Math.max(0, Math.round(stats.avgUsage * (0.5 + Math.random()))) };
  });
...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'update') {
      ...
      return NextResponse.json({ success: true, message: `Skill "${updated.name}" updated`, data: updated });
    }
