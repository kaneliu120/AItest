  ...
  return [
    {
      ...
      status: 'running',
      description: 'Sync SQLite tasks to Apple Calendar, keep calendar and task management in sync',
      lastRun: ago(15),
      nextRun: next(45),
    ...
    },
    {
      ...
      name: 'Health Monitor',
      status: 'running',
      description: 'Real-time monitoring of system CPU/memory usage and tool ecosystem health status',
      lastRun: ago(2),
      nextRun: next(3),
    ...
    },
    {
      ...
      status: 'running',
      description: 'Aggregate finance, task, and outsource data to generate dashboard analytics metrics',
      lastRun: ago(8),
      nextRun: next(52),
    ...
    },
    {
      ...
      description: 'Monitor tool ecosystem health status changes, detect anomalies and trigger alerts',
      lastRun: ago(30),
      nextRun: next(30),
      runCount: 31,
      successRate: 87,
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action === 'status') {
      ...
      // 只StatisticsDevelopmentclassTask(流转toAutomation'sTask)
      ...
      return NextResponse.json({
        ...
        data: {
          ...
          stats: {
            totalModules:     4,
            enabledModules:   4,
            // Only count tasks with type=development (tasks flowing through automation)
    ...
    if (action === 'executions') {
      ...
      // 补充SystemExecuteLog
      const systemExecs = [
        { id: 'sys-1', moduleId: 'health-monitor',    module: 'Health Monitor',  action: 'SystemHealthCheck',   status: 'success', duration: '0.3s', timestamp: new Date(Date.now() - 2  * 60000).toISOString() },
        { id: 'sys-2', moduleId: 'data-aggregator',   module: 'Data Aggregator',  action: 'dataaggregationRefresh',   status: 'success', duration: '1.2s', timestamp: new Date(Date.now() - 8  * 60000).toISOString() },
        { id: 'sys-3', moduleId: 'ecosystem-watcher', module: 'Ecosystem Watcher',  action: 'ToolStatusScan',   status: 'success', duration: '0.8s', timestamp: new Date(Date.now() - 30 * 60000).toISOString() },
        { id: 'sys-4', moduleId: 'task-sync',         module: 'Task Synchronizer',  action: 'Calendar Sync', status: 'success', duration: '2.1s', timestamp: new Date(Date.now() - 15 * 60000).toISOString() },
      ];
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'run-module') {
      ...
      // TriggerdataRefresh(实际调用相Off API)
