...
function validateFinanceTaskPayload(input: unknown):
  ...
  if (body.action !== 'create-task-transaction') return { ok: false, error: 'Unsupported action' };
  if (typeof body.taskId !== 'string' || !body.taskId.trim()) return { ok: false, error: 'Invalid or missing taskId' };
  if (body.type !== undefined && body.type !== 'income' && body.type !== 'expense') return { ok: false, error: 'type must be income or expense' };
  if (body.amount !== undefined && Number.isNaN(Number(body.amount))) return { ok: false, error: 'amount must be a number' };
  return {
    ok: true,
    body: {
  ...
  try {
    ...
    if (parsed.body.action === 'create-task-transaction') {
      ...
      const tx = await financeStore.addTransaction({
        ...
        description: description || `Task link: ${task.title}`,
