...
export async function GET(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!task) return NextResponse.json({ success: false, error: 'Not foundTask' }, { status: 404 });
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!updated) return NextResponse.json({ success: false, error: 'No valid fields to update or task does not exist' }, { status: 400 });
