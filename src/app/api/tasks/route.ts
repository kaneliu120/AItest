...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'update-status') {
      ...
      if (!updated) {
        return NextResponse.json({ success: false, error: 'Task does not exist or update failed' }, { status: 404 });
      }

      return NextResponse.json({
        success: true,
        data: updated,
        message: 'TaskStatusUpdated successfully',
      });
    }

    // default: create
    if (!body.title) {
      return NextResponse.json({ success: false, error: 'TaskTitle cannot be empty' }, { status: 400 });
    }
    ...
    if (!task) {
      return NextResponse.json({ success: false, error: 'Create task failed' }, { status: 500 });
    }

    return NextResponse.json({
      success: true,
      data: task,
      message: 'TaskCreated successfully',
    });
  } catch (error) {
    return NextResponse.json(
      {
